...
export function GoogleSignInButton({
  ...
  const handleClick = () => {
    const googleButton = googleHostRef.current?.querySelector('[role="button"]') as HTMLElement | null;
    googleButton?.click();
  };
  ...
  return (
    <div className="relative w-full">
      ...
      <button
        ...
        disabled={disabled}
        onClick={handleClick}
        className={`w-full py-3 px-4 border-2 rounded-lg transition-colors flex items-center justify-center gap-3 group disabled:opacity-60 disabled:cursor-not-allowed ${
          theme === 'dark'

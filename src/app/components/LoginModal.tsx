...
export function LoginModal({ isOpen, onClose, onSwitchToRegister }: LoginModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (email === 'asesor@gmail.com' && password === '12345') {
      onClose();
      navigate('/asesor');
      return;
    }

    // Verificar si es un analista
    if (email === 'analista@gmail.com' && password === '12345') {
      onClose();
      navigate('/analista');
      return;
    }

    // Cerrar el modal y redirigir al dashboard del cliente
    onClose();
    navigate('/dashboard');
  };
        ...
        <Dialog.Content className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-2xl shadow-2xl w-full max-w-md p-8 z-50 animate-in fade-in zoom-in-95 transition-colors ${
          ...
          <div className="space-y-3 mb-6">
            ...
            >
              ...
              <span className={`font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Continuar con Google</span>
            </button>
